<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $password = "pat".$ran;
        // Add Patient
        $isUserSuccess = $users->insertUser($Emailid, $password, 8);
        if($isUserSuccess){
              //Call function to insert and track if success or not
              $isSuccess = $patient->insertPatient($isUserSuccess,$Fname, $Lname, $Dateofbirth, $AddressLine_1, $AddressLine_2, $Mobileno, $Emailid,$City, $State,$Country,	$ZipCode,$Emergencycontactname,	$Emergencycontactnum,$Relationship, $destination);
              // $specialtyName = $crud->getSpecialtyById($specialty);
                                                                                                                                                                                    
              if($isSuccess){
                SendEmail::SendMail($Emailid, 'Welcome to Dental System', 'You are now part of our Dental System!  Your credentials are  Username: '.$Emailid. ' Password: '.$password  );
