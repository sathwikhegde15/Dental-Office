<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $password = "hyg".$ran;
        // Add Hygienist
        $isUserSuccess = $users->insertUser($Emailid, $password, 9);
        if($isUserSuccess){
              //Call function to insert and track if success or not
              $isSuccess = $hygienist->insertHygienist($isUserSuccess,$Fname, $Lname, $Dateofbirth, $AddressLine_1, $AddressLine_2, $Mobileno, $Emailid,$City, $State,$Country,	$ZipCode,$Certificateno,	$Specialization, $destination);
              // $specialtyName = $crud->getSpecialtyById($specialty);

              if($isSuccess){
                SendEmail::SendMail($Emailid, 'Welcome to Dental System', 'You have successfully added as Hygienist!  Your credentials are  Username: '.$Emailid. ' Password: '.$password  );
